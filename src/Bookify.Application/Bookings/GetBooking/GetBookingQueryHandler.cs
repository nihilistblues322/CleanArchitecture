    ...
    : IQueryHandler<GetBookingQuery, BookingResponse>
{
    public async Task<Result<BookingResponse>> Handle(GetBookingQuery query, CancellationToken cancellationToken)
    {
        ...
        var booking = await connection.QueryFirstOrDefaultAsync<BookingResponse>(sql, new { query.BookingId });

        if (booking is null || booking.UserId != userContext.UserId) return Result.Failure<BookingResponse>(BookingErrors.NotFound);
